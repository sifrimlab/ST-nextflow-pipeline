...
tile_nr =  int(re.findall(r"\d+", sys.argv[3])[0])
codebook = sys.argv[4]
bit_len =  int(sys.argv[5])
threshold = float(sys.argv[6])
image_path_list = [sys.argv[i] for i in range(7, len(sys.argv))]
decodePixelBased(x_dim,y_dim, codebook, bit_len, image_path_list, threshold).to_csv(f"decoded_tiled_{tile_nr}.csv", index=False)
