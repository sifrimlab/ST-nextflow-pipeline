...
seu.run <- function(inputdata, resolution, output_file_prefix){
        ...
        seu.markers <- FindAllMarkers(object = seu_fun, only.pos = TRUE, min.pct = 0, thresh.use = 0)
        print(head(seu.markers))
        top10 <- seu.markers %>% group_by(cluster) %>% top_n(10, avg_log2FC)
        png("clustering_plots.png", width = 1000,height = 500)
        ...
        print(p4)
        dev.off()
        write.csv(top10, "top10.csv", row.names=TRUE)
}
