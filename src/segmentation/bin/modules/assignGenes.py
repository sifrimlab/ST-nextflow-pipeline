...
def assignAllGenesToCells(labeled_image: str, decoded_genes: str, cell_properties: str, filter_unrecognized = True):
    image = io.imread(labeled_image) # slicing = [Y,X]
    decoded_df = pd.read_csv(decoded_genes)
    cell_properties_df = pd.read_csv(cell_properties)
    print(decoded_df.columns, cell_properties_df.columns)
    if filter_unrecognized:
        decoded_df = decoded_df[decoded_df['Gene'].isnull()!=True]

    label_column = []

    # This function finds the closest cell (from cell_properties_df)  to the given excerpt for the decoded_df 
...
if __name__=='__main__':
    ...
    filtered_df = assignAllGenesToCells(labeled_image, decoded_genes, cell_properties)
    print(filtered_df)
